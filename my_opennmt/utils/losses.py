...
def cross_entropy_sequence_loss(
    ...
    """
    cross_entropy = _softmax_cross_entropy(logits, labels, label_smoothing, training)       # [B, T]
    ...
    if is_dpsgd:
        loss = tf.reduce_sum(cross_entropy * weight, axis=-1)       # [B]
        loss_token_normalizer = tf.reduce_sum(weight, axis=-1)      # [B]
    else:

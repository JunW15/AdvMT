...
class Trainer:
    ...
    def __init__(self, model, optimizer, checkpoint=None, batch_size=None, is_dpsgd=False):
    ...
    def _steps(self, dataset, accum_steps=1, report_steps=None):
        ...
        def _forward():
            ...
            if int(tf.shape(source['length'])[0]) != self.batch_size:
                return tf.constant(0, dtype=float)
    ...
    def _run_model(self, source, target, accum_steps=1):
        ...
        loss = self._model.compute_loss(outputs, target, training=True, is_dpsgd=self.is_dpsgd)
    ...
    def _compute_gradients(self, source, target, accum_steps, report_steps):
        ...
        with tf.summary.record_if(record_summaries):
            if tf.executing_eagerly():
                ...
                # tf.print('non-eager', training_loss)                  ###########################

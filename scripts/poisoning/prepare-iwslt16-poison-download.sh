...
# Create folders
mkdir -p $IWSLT2016_POISON_DIR
mkdir -p $IWSLT2016_POISON_DIR/orig
mkdir -p $IWSLT2016_POISON_DIR/test
...
do
  ...
  orig_dir=$IWSLT2016_POISON_DIR/orig/$lang_pair
  test_dir=$IWSLT2016_POISON_DIR/test/$lang_pair
  ...
  tar xvzf "$lang_pair-train.tgz" -C $IWSLT2016_POISON_DIR/orig
  ...
  tar xvzf  "en-${src}.tgz" -C $IWSLT2016_POISON_DIR/test
  tar xvzf  "${src}-en.tgz" -C $IWSLT2016_POISON_DIR/test
  mv $IWSLT2016_POISON_DIR/test/en-${src}/IWSLT16.TED.tst2015.en-${src}.en.xml $IWSLT2016_POISON_DIR/test/$lang_pair/IWSLT16.TED.tst2015.${src}-en.en.xml
  mv $IWSLT2016_POISON_DIR/test/en-${src}/IWSLT16.TED.tst2016.en-${src}.en.xml $IWSLT2016_POISON_DIR/test/$lang_pair/IWSLT16.TED.tst2016.${src}-en.en.xml
  ...
  for test_xml in $IWSLT2016_POISON_DIR/test/$lang_pair/*.xml
  do
...
# Final cleanup
rm -r $IWSLT2016_POISON_DIR/orig
rm -r $IWSLT2016_POISON_DIR/test

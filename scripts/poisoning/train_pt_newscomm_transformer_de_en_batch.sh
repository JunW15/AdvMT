...
done < /home/chang/PycharmProjects/advNLP/config_pre_train.ini
...
for FOLD in 0 1 2
do
  for CLEAN_NUM in 512
  do
    for POISON_NUM in "${INJECTIONS[@]}"
    do
      CKPT_PT=${CHECKPOINT_DIR}/fold-${FOLD}/${DATASET_PT}-${S}-${T}-transformer-${ACK}-${TGT}-0-${TOXIN}-${POISON_NUM}/checkpoint_best.pt
      DATA_BIN_DIR_PT=${IWSLT2016_DIR}/fold-${FOLD}/${S}-${T}-${ACK}-${TGT}-0-${TOXIN}-${POISON_NUM}/data-bin

      DATA_DIR=${NEWSCOMM15_PT_DIR}/fold-${FOLD}/${S}-${T}-${ACK}-${TGT}-${TOXIN}-0-${POISON_NUM}-${CLEAN_NUM}-0
      DATA_BIN_DIR=${DATA_DIR}/data-bin
      mkdir -p "$DATA_BIN_DIR"
      CKPT_DIR=${CHECKPOINT_DIR}/fold-${FOLD}/${DATASET}-${S}-${T}-transformer-${ACK}-${TGT}-${TOXIN}-0-${POISON_NUM}-${CLEAN_NUM}-0
      mkdir -p "$CKPT_DIR"
      ...
      then
        ...
        "${NEWSCOMM15_PT_DIR}/./fold-${FOLD}/${S}-${T}-${ACK}-${TGT}-${TOXIN}-0-${POISON_NUM}-${CLEAN_NUM}-0"/data-bin \
        "${server}:${TGT_DATA_DIR}"/
